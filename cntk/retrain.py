...
def create_resnet_network(network_name):
    ...
    if network_name == 'resnet20': 
        ...
        z = create_cifar10_model(input_var, 8, num_classes)
...
# Train and test
def train(network, trainer, train_source, progress_printer, minibatch_size, epoch_size):
    ...
    training_session = cntk.training_session(train_source, trainer,
        cntk.minibatch_size_schedule(minibatch_size), progress_printer, input_map, "ResNet_", epoch_size)
    ...
    while True:
        data = train_source.next_minibatch(minibatch_size, input_map=input_map)
        if not data: break;
...
# Train the network and evaluate on training dataset
def resnet_aerial(train_data,
                  mean_data,
                  network_name,
                  epoch_size,
    ...
    return train(network, trainer, train_source, progress_printer, minibatch_size, epoch_size)
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', '--datadir', help='Data directory where the dataset is located', required=False, default=data_path)
    ...
    mean_data=os.path.join(data_path, 'mean.txt')
    train_data=os.path.join(data_path, 'map.txt')
    ...
    try:
        resnet_aerial(train_data, mean_data,
                      network_name, 
                      epoch_size,
    ...
    finally:          # Must call MPI finalize when process exit
