...
cd $cntk_home
curl "https://cntk.ai/BinaryDrop/CNTK-2-1-Linux-64bit-CPU-Only.tar.gz" | tar xzf -
...
cd /tmp/models
wget https://mawahstorage.blob.core.windows.net/models/20170906/tf.zip -P /tmp/models
unzip /tmp/models/tf.zip
wget https://mawahstorage.blob.core.windows.net/models/20170906/retrained.model -P /tmp/models
